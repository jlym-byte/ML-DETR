...
class RTDETRDetectionModel(DetectionModel):
    ...
    Attributes:
        cfg (str): The configuration file path or preset string. Default is 'ml-detr.yaml'.
    ...
    def __init__(self, cfg='ml-detr.yaml', ch=3, nc=None, verbose=True):
        """
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False):
        ...
        if isinstance(head.f, int):
            head_input = x if head.f == -1 else y[head.f]
            if not isinstance(head_input, (list, tuple)):
                head_input = [head_input]
        else:
            head_input = [x if j == -1 else y[j] for j in head.f]
        x = head(head_input, batch)  # head inference
        return x
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    """Parse a YOLO/RT-DETR model.yaml dictionary into a PyTorch model."""
    import ast
    ...
    base_modules = (
        ...
        BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.Conv2d, nn.ConvTranspose2d, DWConvTranspose2d,
        C3x, RepC3, ConvNormLayer, TOGLSR, ECIE,
    )
    repeat_modules = (
        BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, ECIE,
    )
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if m is SSMamba:
            ...
            decoder_ch = ch[f] if isinstance(f, int) else [ch[x] for x in f]
            if isinstance(decoder_ch, int):
                decoder_ch = [decoder_ch]
            elif isinstance(decoder_ch, list) and decoder_ch and isinstance(decoder_ch[0], list):
                decoder_ch = sum(decoder_ch, [])
            args.insert(1, decoder_ch)
        elif m is Blocks:
